...
import shutil
import sys
from listaDict import listaPastas

diretorio = './'
diretorioBack = ''
diretorioFront = ''

cards=listaPastas(diretorio)
...
for card in cards:
    ...
    caminho_arquivo = f'./{diretorio}/{arquivoGPS}'
    ...
    for lista in tudao:
        ...
        if(not 'semVideo' in nome_arquivo[1]):
            if("B" in nome_arquivo[2]):
                nome_arquivo=diretorio+'/'+nome_arquivo[0]+'-'+nome_arquivo[1]+'-Back.mp4'
            else:
                nome_arquivo=diretorio+'/'+nome_arquivo[0]+'-'+nome_arquivo[1]+'-Frnt.mp4'
    ...
    def colum_names(matriz):
        vetor = []
        vetor = ['DRIVER', 'LONG', 'LAT', 'DAY', 'DAY_CORRIGIDO', '03:00:00', 'TRIP', 'ID',
                'PR', 'H', 'M', 'S', 'TIME_ACUM', 'SPD_KMH', 'SPD_MPH', 'ACEL_MS2', 'HEADING', 'ALTITUDE_FT',
    ...
    for lista in tudao:
        matriz = []
        colum_names(matriz)
        arquivo = diretorio+'/'+lista[0].replace(".mp4", ".csv")
        novoNome = ""
        if (not "F" in arquivo):
    ...
    # Movimentacao
    pastaVideos = diretorio+'/'+'videosConcatenados'
    # Verificar se a pasta não existe antes de criar
    if not os.path.exists(pastaVideos):
        os.makedirs(pastaVideos)

    pastaPlanilhas = diretorio+'/'+'gpsFiles'
    # Verificar se a pasta não existe antes de criar
    if not os.path.exists(pastaPlanilhas):
        os.makedirs(pastaPlanilhas)

    files=os.listdir(diretorio)
    for i in files:
        i=diretorio+'/'+i
        if(("NO" in i) and ((".txt" in i) or (".csv" in i))):
            shutil.move(i, pastaPlanilhas)
        elif(("NO" in i) and (".mp4" in i)):
            shutil.move(i, pastaVideos)
